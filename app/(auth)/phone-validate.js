...
export default function PhoneValidate() {
    ...
    const handleSendCode = async () => {
        try {
            ...
            Toast.show({
                type: 'info',
                text1: 'Sending Verification Code. Please wait.',
                position: 'bottom',
                autoHide: false
            });
    ...
    const handleVerify = async () => {
        try {
            ...
            setProcessing(true);
            Keyboard.dismiss();
            const firebaseUser = await firebaseAuthService.auth.currentUser;
            await firebaseUser.linkWithPhoneNumber(phone.number);
